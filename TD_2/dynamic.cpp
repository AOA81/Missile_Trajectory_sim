...
float thrust_misalignment_d;
float thrust_misalignment_theta;

float mass_deflection;
float velocity_0_deflection;
float theta_0_deflection;
float psi_0_deflection;
float phi_0_deflection;
...
float * dynamic(float state[13], float time)
{
    ...
    u_wind = cos(psi)*cos(theta)*W_x + cos(theta)*sin(psi)*W_y - sin(theta)*W_z;
    v_wind = (sin(phi)*sin(theta)*cos(psi)-cos(phi)*sin(psi))*W_x +
             (sin(phi)*sin(theta)*sin(psi)+cos(phi)*cos(psi))*W_y + sin(phi)*cos(theta)*W_z;
    w_wind = (cos(phi)*sin(theta)*cos(psi)+sin(phi)*sin(psi))*W_x +
             (cos(phi)*sin(theta)*sin(psi)-sin(phi)*cos(psi))*W_y + cos(phi)*cos(theta)*W_z;
    ...
    velocity = sqrt(u_w*u_w + v_w*v_w + w_w*w_w);

    Ma = velocity / velocity_of_sound;

    alpha = atan(w_w / u_w);
    beta = asin(v_w / sqrt(u_w*u_w + v_w*v_w + w_w*w_w));
    ...
    m_yb_a += C_z * (Xg_current - 0.9381) / characteristic_length; //修正静稳定力矩系数
    ...
    M_xb = 0;//debug
    M_yb = m_yb * dyn_pressure * characteristic_area * characteristic_length + thrust * thrust_misalignment_d * sin(thrust_misalignment_theta * PI / 180);
    M_zb = m_zb * dyn_pressure * characteristic_area * characteristic_length + thrust * thrust_misalignment_d * cos(thrust_misalignment_theta * PI / 180);
